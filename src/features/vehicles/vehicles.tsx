...
export default function VehiclesLayer({ filter }: { filter?: FilterSpecification }) {
    ...
    const vehicleFeatures = useMemo(() => ({
        ...
        features: vehicles.map((v) => ({
            ...
            geometry: {
                type: 'Point' as const,
                coordinates: [v.vehicle.location.lon, v.vehicle.location.lat],
            },
            properties: {
    ...
    const vehicleBearings = useMemo(() => {
        ...
        const collection = {
            ...
            features: vehicles.filter(v => v.vehicle.bearing !== undefined).map(v => ({
                type: 'Feature',
                geometry: { type: 'Point', coordinates: [v.vehicle.location.lon, v.vehicle.location.lat] },
                properties: {
                    rotate: v.vehicle.bearing,
                    tripId: v.tripId,
                    "icon-image": v.vehicle.status === "STOPPED_AT" ? "v-bearing-stopped" : "v-bearing-moving"
                }
            }))
